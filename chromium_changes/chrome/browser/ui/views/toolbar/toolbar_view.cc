...
void ToolbarView::Init() {
  ...
  // הגדרת הגודל ההתחלתי (נגיד רוחב 200 פיקסלים, גובה 30)
  ai_textfield->SetPreferredSize(gfx::Size(1, 30));

  // לאפשר לתיבה להתרחב בעזרת FlexLayout (אם ה־container_view_ משתמש
  // ב-FlexLayout)
  ...
  //// צבע טקסט ורקע (optional אם לא רואים את הטקסט)
  //ai_textfield->SetTextColor(SK_ColorWHITE);
  //ai_textfield->SetBackgroundColor(SK_ColorBLACK);

  //// אפשרות: להבטיח פוקוס בלחיצה
  //ai_textfield->SetFocusBehavior(views::View::FocusBehavior::ALWAYS);

  //// לבסוף, הוסף ל־container_view_
  ...
  //chat_toggle_button_->SetImageModel(
  //    views::Button::STATE_NORMAL,
  //    ui::ImageModel::FromImageSkia(
  //        *ui::ResourceBundle::GetSharedInstance().GetImageSkiaNamed(
  //            IDR_CHAT_LOGO)));

  //chat_toggle_button_->SetTooltipText(u"Open AI Chat");
  //chat_toggle_button_->SetAccessibleName(u"Open AI Chat");
  //chat_toggle_button_->SetPosition(
...
void ToolbarView::SendAiTaskToServer(const std::u16string& text) {
  ...
  // הוספת base::WeakPtr הכרחית כדי להימנע מקריסה אם ToolbarView יימחק לפני
  // התגובה.
  ...
  LOG(INFO) << "AI task sent: " << text;
}

#include "content/public/browser/web_contents.h"
#include "url/gurl.h"

void ToolbarView::OnServerResponse(std::unique_ptr<std::string> response_body) {
  LOG(INFO) << "Server responded. Entered callback.";

  if (response_body && !response_body->empty()) {
    std::string escaped_body = *response_body;

    // Remove surrounding quotes if present
    if (!escaped_body.empty() && escaped_body.front() == '"' &&
        escaped_body.back() == '"') {
      escaped_body = escaped_body.substr(1, escaped_body.length() - 2);
    }

    // Escape special characters for safe HTML embedding
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "&", "&amp;");
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "<", "&lt;");
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, ">", "&gt;");
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "\"", "&quot;");
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "'", "&#39;");

    // Convert newlines (\n) to <br> for proper HTML formatting
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "\n", "<br>");
    base::ReplaceSubstringsAfterOffset(&escaped_body, 0, "\\n", "<br>");
    ...
    while ((pos = escaped_body.find("**", pos)) != std::string::npos) {
      escaped_body.replace(pos, 2, "<b>");
      pos = escaped_body.find("**", pos);
      if (pos != std::string::npos) {
        escaped_body.replace(pos, 2, "</b>");
        pos += 4;  // Move past the closing tag
      }
    }

    // HTML content with line breaks, bold text support, and dark mode
    std::string html_content =
        "<html>"
        "<head><meta charset='UTF-8'><title>AI Result</title>"
        "<style>"
        "body { font-family: Arial, sans-serif; margin: 0; padding: 20px; "
        "background-color: #121212; color: #e0e0e0; }"
        "h1 { color: #ffffff; text-align: center; font-size: 24px; }"
        "div { background: #1e1e1e; padding: 20px; border-radius: 10px; "
        "box-shadow: 0px 0px 15px rgba(255,255,255,0.1); max-width: 90%; "
        "margin: auto; }"
        "pre { white-space: pre-wrap; word-wrap: break-word; font-size: 18px; "
        "color: #bb86fc; line-height: 1.5; }"
        "b { color: #ffcc00; }"  // Bold text color
        "</style>"
        "</head>"
        "<body>"
        "<h1>AI Response</h1>"
    ...
    std::string data_url = "data:text/html;charset=utf-8," +
                           base::EscapeQueryParamValue(html_content, false);

    // Open new tab and display formatted response
    browser_->OpenURL(
        content::OpenURLParams(GURL(data_url), content::Referrer(),
                               WindowOpenDisposition::NEW_FOREGROUND_TAB,
                               ui::PAGE_TRANSITION_TYPED, false),
        base::NullCallback());

    LOG(INFO) << "Opened a new tab with AI response.";
  } else {
    LOG(ERROR) << "No response or empty response from server.";
  }
}
