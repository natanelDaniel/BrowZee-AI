...
from browzee_agent.agent.message_manager.views import ManagedMessage

# from lmnr.sdk.decorators import observe
from pydantic import BaseModel, ValidationError

from browzee_agent.agent.gif import create_history_gif
from browzee_agent.agent.memory.service import Memory
from browzee_agent.agent.memory.views import MemoryConfig
from browzee_agent.agent.message_manager.service import MessageManager, MessageManagerSettings
from browzee_agent.agent.message_manager.utils import convert_input_messages, extract_json_from_model_output,is_model_without_tool_support, save_conversation
from browzee_agent.agent.prompts import AgentMessagePrompt, PlannerPrompt, SystemPrompt
from browzee_agent.agent.views import (
...
from browzee_agent.browser.browser import Browser
from browzee_agent.browser.context import BrowserContext
from browzee_agent.browser.views import BrowserState, BrowserStateHistory
from browzee_agent.controller.registry.views import ActionModel
from browzee_agent.controller.service import Controller
from browzee_agent.dom.history_tree_processor.service import (
	DOMHistoryElement,
	HistoryTreeProcessor,
)
from browzee_agent.exceptions import LLMException
from typing_extensions import Annotated, TypedDict
from browzee_agent.telemetry.service import ProductTelemetry
from browzee_agent.telemetry.views import (
	AgentEndTelemetryEvent,
	AgentRunTelemetryEvent,
	AgentStepTelemetryEvent,
)
from browzee_agent.utils import check_env_variables, time_execution_async, time_execution_sync

import socket
from fastapi import FastAPI, WebSocket
from browzee_agent.app_services import app, send_to_websockets, websocket_endpoint, wait_for_user_response
load_dotenv()
...
class need_user(TypedDict):
	is_need_user: Annotated[bool, 'there is a need for user input']
	question: Annotated[Optional[str], 'the question to ask the user - what is missing to complete the task']
...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		self.send_to_websockets_flag = send_to_websockets_flag
	...
	async def step(self, step_info: Optional[AgentStepInfo] = None, user_responses=[]) -> None:
		...
		try:
			...
			if self.settings.planner_llm and self.state.n_steps % self.settings.planner_interval == 0:
				plan = await self._run_planner(user_responses)
			...
			try:
				model_output = await self.get_next_action(input_messages, user_responses)
				if (not model_output.action or not isinstance(model_output.action, list) or all(action.model_dump() == {} for action in model_output.action)):
					...
					model_output = await self.get_next_action(retry_messages, user_responses)
	...
	async def get_next_action(self, input_messages: list[BaseMessage], user_responses: list[tuple[str, str]]) -> AgentOutput:
		...
		if self.memory:
			for msg in reversed(self.memory.message_manager.state.history.messages):
				if isinstance(msg, ManagedMessage) and msg.metadata.message_type == 'memory':
					input_messages.insert(0, SystemMessage(content=msg.message.content))
					break
		#  remove images from input_messages
		image = None
			...
			if isinstance(msg.content, list):
				for content in msg.content:
					if content['type'] == 'image_url':
						...
						input_messages.remove(msg)

		# if self.vision_model:
		# 	answer = await self.vision_model.ainvoke(image)
		# 	input_messages.append(AIMessage(content=answer.content))
		if user_responses:
	...
	async def _log_agent_run(self, send_to_websockets_flag: bool = False) -> None:
		"""Log the agent run"""
		logger.info(f'🚀 Starting task: {self.task}')
		if send_to_websockets_flag:
			await send_to_websockets(f'🚀 Starting task: {self.task}')
	...
	async def run(
		...
		from browzee_agent.utils import SignalHandler
	...
	def _verify_llm_connection(self) -> bool:
		...
		try:
			...
			if required_keys:
				...
				pass
	async def _run_planner(self, user_responses: list[tuple[str, str]]) -> Optional[str]:
		...
		if self.memory:
			for msg in reversed(self.memory.message_manager.state.history.messages):
				if isinstance(msg, ManagedMessage) and msg.metadata.message_type == 'memory':
					planner_messages.insert(0, SystemMessage(content=msg.message.content))
					break
	...
	async def run_interactive(self, max_steps: int = 100, on_step_start: AgentHookFunc | None = None, on_step_end: AgentHookFunc | None = None, send_to_websockets_flag: bool = True,
) -> AgentHistoryList:
		...
		try:
			...
			for step in range(max_steps):
				...
				step_info = AgentStepInfo(step_number=step, max_steps=max_steps)
				await self.step(step_info, self.user_responses)
				if on_step_end is not None:
					await on_step_end(self)
				...
				if step > 0 and self.interactive:
				...
				if flag:
					...
					self._process_user_response(user_response, question)
			...
			if self.settings.generate_gif:
				...
				create_history_gif(task=self.task, history=self.state.history, output_path=output_path)
	# Placeholder methods for user interaction

	async def _needs_user_input(self) -> tuple[bool, Optional[str]]:
		"""Determine if user input is needed using an AI model and identify missing information"""
		...
		# חפש את החלון של הדפדפן לפי הכותרת
		windows = gw.getWindowsWithTitle("Browzee")  # או "Brave", "Firefox", וכו'
		if windows:
			...
			# צילום רק של האזור הזה
			screenshot = pyautogui.screenshot(region=(left, top, width, height))

			print("✔ Screenshot saved.")
		else:
			print("❌ No browser window found.")
		# Convert input to a list of BaseMessages
		message_text = f"""
		Analyze the following agent state and image, and determine if the agent need addinoal information like: filling boxes like mail or locations or phone
		prefare to not ask the user. if you are brining a sugeestions -
		 like if the usae asks give me resturants - 
		 you can suggest to the agent to ask for the type of food - and location and suggest few options
		 than you can ask the user if he wants to see the menu or the location on the map, and to make a reservation and ask 
		 for preferred time and date and other details. (just example)
		Task: "{self.task}
		last result: {serialize_state(self.state.last_result)}
		consecutive_failures: {self.state.consecutive_failures}

		User responses so far: {serialize_state(self.user_responses)}

		Should the agent ask the user a question now? If yes, what question?
		Return JSON in the following format:
		  "is_need_user": true | false,
		...
		if response['parsed']['is_need_user']:
			return response['parsed']['is_need_user'], response['parsed']['question']
		return response['parsed']['is_need_user'], ''

	def _generate_question_for_user(self, missing_info: Optional[str]) -> str:
		"""Generate a question to ask the user based on missing information"""
		if missing_info:
			return f"Could you please provide the following information: {missing_info}?"
		return "What would you like to do next?"


	def _process_user_response(self, response: str, question: str) -> None:
		"""Directly append the user's response to the agent's existing state or context"""
		...
		self.memory.message_manager.state.history.messages += [HumanMessage(content=f"User was asked: {question}\nUser answered: {response}")]
	async def _get_user_response(self, question: str) -> str:
		"""שלח שאלה למשתמש דרך WebSocket וחכה לתשובה"""
		return await wait_for_user_response(question)
