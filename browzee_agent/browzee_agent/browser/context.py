...
from browzee_agent.browser.views import (
	BrowserError,
	BrowserState,
	TabInfo,
	URLNotAllowedError,
)
from browzee_agent.dom.service import DomService
from browzee_agent.dom.clickable_element_processor.service import ClickableElementProcessor

from browzee_agent.dom.views import DOMElementNode, SelectorMap
...
import base64
def background_screenshot(hwnd, width, height) -> str:
...
class BrowserContextConfig:
	...
	minimum_wait_page_load_time: float = 0.2
	wait_for_network_idle_page_load_time: float = 0.3
	maximum_wait_page_load_time: float = 2
...
class BrowserContext:
	...
	async def _initialize_session(self):
		...
		if not current_page:
			i = 0
			flag = False
			while i < len(pages) and not flag:
				if (
					pages
					and pages[i].url
					and not pages[i].url.startswith('chrome://')  # skip chrome internal pages e.g. settings, history, etc
					and not pages[i].url.startswith('chrome-extension://')  # skip hidden extension background pages
				):
	...
	async def _get_updated_state(self, focus_element: int = -1) -> BrowserState:
		...
		try:
			...
			if pages:
				...
				page = await self.get_agent_current_page(session)
	...
	async def take_screenshot(self, full_page: bool = False) -> str:
		...
		screenshot_b64 = background_screenshot(hwnd, 1280, 780)
		await self.remove_highlights()
