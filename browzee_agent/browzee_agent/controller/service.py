...
from browzee_agent.agent.views import ActionModel, ActionResult
from browzee_agent.browser.context import BrowserContext
from browzee_agent.controller.registry.service import Registry
from browzee_agent.controller.views import (
	ClickElementAction,
	CloseTabAction,
	DoneAction,
	DragDropAction,
...
from browzee_agent.utils import time_execution_sync
from browzee_agent.app_services import send_to_websockets
...
class Controller(Generic[Context]):
	def __init__(
		...
		async def open_tab(params: OpenTabAction, browser: BrowserContext):
			...
			await browser.get_agent_current_page()
			msg = f'🔗  Opened new tab with {params.url}'
			logger.info(msg)
			await send_to_websockets("Opened new tab")
			return ActionResult(extracted_content=msg, include_in_memory=True)
		...
		async def extract_content(
			...
			try:
				...
				logger.info(msg)
				await send_to_websockets("Extracted content from page")
				return ActionResult(extracted_content=msg, include_in_memory=True)
			except Exception as e:
				...
				for locator in locators:
					try:
						...
						if await locator.count() > 0 and await locator.first.is_visible():
							...
							logger.info(msg)
							await send_to_websockets("Scrolled to text")
							return ActionResult(extracted_content=msg, include_in_memory=True)
					except Exception as e:
				...
				for frame in page.frames:
					try:
						...
						if dropdown_info:
							...
							await send_to_websockets(msg + f' in frame {frame_index}')
