...
from browzee_agent.agent.message_manager.utils import convert_input_messages, extract_json_from_model_output, save_conversation
...
from browzee_agent.utils import time_execution_async, time_execution_sync

import socket
from fastapi import FastAPI, WebSocket
from browzee_agent.app_services import app, send_to_websockets, websocket_endpoint, wait_for_user_response
from browzee_agent.MemoryManager import MemoryManager
load_dotenv()
...
class Agent(Generic[Context]):
	...
	async def run_interactive(self, max_steps: int = 100) -> AgentHistoryList:
	...
	async def _needs_user_input(self) -> tuple[bool, Optional[str]]:
		...
		windows = gw.getWindowsWithTitle("Chromium")  # או "Brave", "Firefox", וכו'
		if windows:
		...
		Task: "{self.task}
		lasr result: {serialize_state(self.state.last_result)}
		consecutive_failures: {self.state.consecutive_failures}
